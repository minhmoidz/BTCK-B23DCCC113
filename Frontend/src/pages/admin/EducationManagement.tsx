...
const API_BASE_URL = 'https://btck-123.onrender.com/api'; 
...
const EducationManagement: React.FC = () => {
  ...
  const handleSchoolSubmit = async (values: School) => {
    try {
      ...
      console.error('Lỗi khi lưu thông tin trường:', error);
      message.error(`Không thể lưu thông tin trường: ${(error as any).response?.data?.message || (error as any).message}`);
    }
  };
