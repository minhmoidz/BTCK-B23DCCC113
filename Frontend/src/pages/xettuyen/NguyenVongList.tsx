...
const NguyenVongList: React.FC<NguyenVongListProps> = ({ userId }) => {
  ...
  const fetchHoSoList = async () => {
    ...
    try {
      ...
      const response = await axios.get('https://btck-123.onrender.com/api/hoso', {
        headers: {
  ...
  const columns = [
    ...
    },
    {
      ...
      onFilter: (value: any, record: HoSo) => record.phuongThuc === String(value)
    },
    {
      title: 'Trạng thái',
      dataIndex: 'trangThai',
      ...
      onFilter: (value: any, record: HoSo) => record.trangThai === String(value)
    },
    {
      title: 'Ngày nộp',
      dataIndex: 'createdAt',
