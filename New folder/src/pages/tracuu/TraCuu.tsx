...
const AdmissionCalculator: React.FC = () => {
  ...
  const API_BASE_URL = 'http://localhost:3000/api/chance';
...
>>>>>>> temp-remote/main
    ...
    if (admissionTypeForCalc === 'thpt') {
      const thptScores = getSubjectScoresArray();
      if (!thptScores) return null;
      payload.scores = thptScores;
      payload.priorityGroup = priorityGroup;
      payload.priorityArea = priorityArea;
    } else if (admissionTypeForCalc.startsWith('dgnl_')) {
      if (dgnlScore === null) return null;
      payload.scores = { score: dgnlScore };
    } else if (admissionTypeForCalc === 'dgtd') {
      if (dgtdScore === null) return null;
      payload.scores = { score: dgtdScore };
    }
    ...
    try {
      ...
      console.error(`Lỗi tính toán cho ngành ${major.majorCode}:`, error);
      return null;
    }
  };
